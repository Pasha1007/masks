  ...
  constructor({ resources, screenVersion, slotConfig, renderer }: SlotType) {
    ...
    if (slotConfig !== "EMPTY") {
      ...
      const defaultOffsetY = (ScreenVersion.MOBILE)
        ? this.spineSlot.h + screenConfig.offsetY - 9
        : this.spineSlot.h + screenConfig.offsetY

      this.spineSlot.move({
        x: (this.slotW - this.spineSlot.width) / 3.5 + defaultOffsetX,
        y: (this.slotH - this.spineSlot.height) / 1.2 + defaultOffsetY + this.spineSlot.height,
      });
