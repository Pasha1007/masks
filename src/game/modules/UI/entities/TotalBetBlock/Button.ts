...
export class Button extends Container {
  ...
  constructor({ config, onClick }: ButtonType) {
    ...
    buttonsContainer.addChild(this.button, this.buttonHover, this.text);
    if (this.key === "next" || this.key === "prev") {
      ...
      if (this.key === "prev") arrow.rotation = -1.5
      buttonsContainer.addChild(arrow)
    }

    buttonsContainer.x = this.button.width / 12;
    buttonsContainer.y = this.button.height / 3;
